...
async function fetcher(
    ...
    try {
        ...
        return convertAxiosErrorToBaseResponse<Page<TimelineSprintResponseDTO>>(err)
    }
}
...
export function useGetTimelineProjectSprints(projectId: string) {
    ...
    const triggerGetTimelineProjectSprints = async (page: number, size: number, year: number): Promise<BaseResponse<Page<TimelineSprintResponseDTO>>> => {
