...
export function ProductBacklogItem({ backlog }: { backlog: ProductBacklog }) {
    const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: backlog.id })
    ...
    useEffect(() => {
        if (backlog.assigneeId) {
            triggerFindUser(backlog.assigneeId).then((res) => {
                ...
                <CardContent className="p-3">
                    <div className="flex items-center gap-4 w-full">
                        ...
                        <div className="flex-1">
                            <div className="font-medium text-sm truncate">{backlog.title}</div>
                        </div>
                        ...
                        <Badge className={`text-xs ${getPriorityColor(backlog.priority)}`}>
                            {getPriorityLabel(backlog.priority)}
                        </Badge>

                        <Badge className={`text-xs ${getStatusColor(backlog.status)}`}>
